...
说明：
- 当前版本为了消除“旧位置残影”，无检测帧的目标可视化最多只保留 1 帧；
- `--obj-temporal-hold-frames` 仍会影响轨迹重关联稳定性，但不会无限延长画面残留。
...
1. `--unknown-profile balanced -> loose`
2. 遮挡很重时优先 `loose`，并观察是否把手也误吸进 unknown

若 unknown 太多（误报）：

1. `--unknown-profile balanced -> strict`
2. 若仍多，直接关闭 unknown 回退：`--no-unknown-roi-fallback`
3. 继续使用已知类别白名单 + ROI 小物体策略作为主路径
...
2. `--roi-partial-profile`
3. `--roi-object-max-area-ratio`

推荐方向：

1. 重复仍多：`--roi-partial-profile balanced -> strict`
2. ROI 经常检出大块局部：降低 `--roi-object-max-area-ratio`
3. ROI 真正小目标被误杀：`--roi-partial-profile strict -> balanced -> loose`
