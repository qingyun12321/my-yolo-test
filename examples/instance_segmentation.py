...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run YOLO instance segmentation on a webcam and save output as mp4."
    )
    parser.add_argument(
        "--model",
        default="models/yolo26n-seg.pt",
        help="Model path or name (default: models/yolo26n-seg.pt).",
    )
    parser.add_argument(
        "--source",
        default=None,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output mp4 path. Default: output/instance_segmentation_<timestamp>.mp4",
    )
    parser.add_argument(
        "--no-preview",
        action="store_true",
...
def build_output_path(output_arg: str | None) -> Path:
    ...
    return output_dir / f"instance_segmentation_{timestamp}.mp4"
